...
race.unemployment.codes = tribble(
  ~series_id, ~table.name,
  'LNS14000003', 'unemployment.rate.white',
  'LNS14000006', 'unemployment.rate.black',
  'LNS14000009', 'unemployment.rate.latino',
  'LNS14032183', 'unemployment.rate.asian',
)

race.unemployment = ln.data %>% 
  right_join(race.unemployment.codes) %>% 
  filter(period != 'M13') %>% 
  mutate(
    report.date = glue('{year}-{str_replace(period, "M", "")}-01'),
    report.date = ymd(report.date)
...
plot.race.unemployment = race.unemployment %>% 
  select(report.date, table.name, value) %>% 
  mutate(table.name = str_replace(table.name, 'unemployment.rate.', '')) %>% 
  ggplot(aes(report.date, value, color = table.name)) +
  geom_line() +
  geom_point(
    data = . %>% 
      group_by(table.name) %>% 
  ...
  geom_hline(yintercept = 14.7, linetype = 'dashed', color = 'red') +
  # facet_wrap(. ~ table.name) +
  ylab('Seasonally adjusted unemployment rate') +
  theme_minimal()

plot.race.unemployment

ggsave('plots/unemployment-rate-by-race.png', width = 11, height = 8)
